local description = [[Praca polega na polowaniu i skórowaniu dzikich zwierząt.
Użyj broni by zabić zwierzę, a następnie podejdź i opraw je.
Im bardziej się postarasz, tym lepszy ekwipunek otrzymasz!
Bądź ostrożny - zwierzęta są całkiem agresywne.
]]
local requirements = "licencja na broń"
local rewards = ""
...
function showJobGUI()
	...
	rewards = "$"..tostring(exports["pd-jobsettings"]:getJobData("hunter", "skin_reward"))..": za każdą skórę"
	rewards = rewards.."\n$"..tostring(exports["pd-jobsettings"]:getJobData("hunter", "distance_reward"))..": bonus za dystans zestrzelenia"
	rewards = rewards.."\n$"..tostring(exports["pd-jobsettings"]:getJobData("hunter", "fast_skin_reward"))..": bonus za szybkie oprawienie"
	...
	addEventHandler("onClientClickButton", acceptButton, function()
		...
		if addData["license:weapon"] then
			startJob()
			hideJobGUI()
		else
			exports["pd-hud"]:showPlayerNotification("Nie posiadasz licencji na broń!", "error")
		end
	end)
	...
	cancelButton = exports["pd-gui"]:createButton("Anuluj", math.floor(windowPos.x+buttonSize.w/2+50/zoom), math.floor(windowPos.y+windowPos.h-buttonSize.h*1.5), buttonSize.w, buttonSize.h)
