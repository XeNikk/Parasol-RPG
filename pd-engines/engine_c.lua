...
--]]
ENGINE_ENABLED = true
ENGINE_VOLUME_MASTER = 0.15 -- mnożnik głośności
		...
		if isElement(vehicle) then
			...
			if engine then
				...
				if getVehicleEngineState(vehicle) == true and distance < ENGINE_SOUND_DISTANCE*2 then
					...
					-- ALS
					if data.wasRevLimited then -- jeśli gazujemy
						...
						if changedGear == "up" and math.random(1, 4) == 1 then -- losowo przy zmianie biegów
							if data.als then 
								data.activeALS = true
							end
					...
					-- prowadzimy pojazd: możemy go aktualizować
